 ...
 */
public class CorreosView extends FrameView {
    
    public CorreosView(SingleFrameApplication app) {
        super(app);
    ...
    public void newRecord() {
        correos.Contactos C = new correos.Contactos();
    ...
    public void deleteRecord() {
        ...
        List<correos.Contactos> toRemove = new ArrayList<correos.Contactos>(selected.length);
        for (int idx=0; idx<selected.length; idx++) {
            correos.Contactos C = list.get(masterTable.convertRowIndexToModel(selected[idx]));
            toRemove.add(C);
    ...
    public void newDetailRecord() {
        ...
        correos.Contactos C = list.get(masterTable.convertRowIndexToModel(index));
        Collection<correos.Correos> cs = C.getCorreosList();
        if (cs == null) {
            cs = new LinkedList<correos.Correos>();
            C.setCorreosList((List)cs);
        }
        correos.Correos c = new correos.Correos();
        entityManager.persist(c);
        c.setContactos(C);
        cs.add(c);
    ...
    public void deleteDetailRecord() {
        ...
        List<correos.Correos> toRemove = new ArrayList<correos.Correos>(selected.length);
        for (int idx=0; idx<selected.length; idx++) {
            selected[idx] = detailTable.convertRowIndexToModel(selected[idx]);
            int count = 0;
            Iterator<correos.Correos> iter = cs.iterator();
            while (count++ < selected[idx]) iter.next();
            correos.Correos c = iter.next();
            toRemove.add(c);
            entityManager.remove(c);
        }
    ...
    private class SaveTask extends Task {
        ...
        @Override protected Void doInBackground() {
            try {
                ...
                rex.printStackTrace();
                entityManager.getTransaction().begin();
                List<correos.Contactos> merged = new ArrayList<correos.Contactos>(list.size());
                for (correos.Contactos C : list) {
    ...
    public void showAboutBox() {
        if (aboutBox == null) {
            JFrame mainFrame = CorreosApp.getApplication().getMainFrame();
            aboutBox = new CorreosAboutBox(mainFrame);
            aboutBox.setLocationRelativeTo(mainFrame);
        }
        CorreosApp.getApplication().show(aboutBox);
    }
    ...
    private void initComponents() {
        ...
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(correos.CorreosApp.class).getContext().getResourceMap(CorreosView.class);
        ...
        javax.swing.ActionMap actionMap = org.jdesktop.application.Application.getInstance(correos.CorreosApp.class).getContext().getActionMap(CorreosView.class, this);
    ...
    private java.util.List<correos.Contactos> list;
